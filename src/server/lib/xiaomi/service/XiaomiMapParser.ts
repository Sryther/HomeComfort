 ...
 */
class XiaomiMapParser {
    private readonly buf: Buffer;
    private offset: number;
    private mapId: number | undefined;
    private robotStatus: Buffer | undefined;
    private mapHead: Buffer | undefined;
    private navigateTarget: { position: number[] | undefined } | undefined;
    private history: any[] | undefined;
    private chargeStation: { orientation: number; position: number[] | undefined } | undefined;
    private virtual_wall: any[] | undefined;
    private noGoArea: any[] | undefined;
    private cleanArea: any[] | undefined;
    private points: any[] | undefined;
    private realtimePose: { position: number[] | undefined } | undefined;
    private img: any;
    private rooms: any;

    public static MAX_MAP_HEIGHT = XiaomiMapParser.convertFloat(20); // 4000
    public static POSITION_UNKNOWN = 1100;
     ...
     */
    readFloatPosition(buf: Buffer, offset: number) {
        // noinspection PointlessArithmeticExpressionJS
        const x = buf.readFloatLE(offset + 0);
        const y = buf.readFloatLE(offset + 4);
        if (x === XiaomiMapParser.POSITION_UNKNOWN || y === XiaomiMapParser.POSITION_UNKNOWN) {
            return;
        }
        return [
            XiaomiMapParser.convertFloat(x),
            XiaomiMapParser.MAX_MAP_HEIGHT - XiaomiMapParser.convertFloat(y)
        ];
    }
    ...
    */
    ...
    parse() {
        ...
        if (featureFlags & 0b000000000010000) {
            ...
            for (let i = 0; i < wall_num; i++) {
                ...
                if (body[0] === body[2] && body[1] === body[3] && body[4] === body[6] && body[5] === body[7]) {
                    //is wall
                    let x1 = Math.round(XiaomiMapParser.convertFloat(body[0]));
                    let y1 = Math.round(XiaomiMapParser.MAX_MAP_HEIGHT - XiaomiMapParser.convertFloat(body[1]));
                    let x2 = Math.round(XiaomiMapParser.convertFloat(body[4]));
                    let y2 = Math.round(XiaomiMapParser.MAX_MAP_HEIGHT - XiaomiMapParser.convertFloat(body[5]));
                    ...
                    let x1 = Math.round(XiaomiMapParser.convertFloat(body[0]));
                    let y1 = Math.round(XiaomiMapParser.MAX_MAP_HEIGHT - XiaomiMapParser.convertFloat(body[1]));
                    let x2 = Math.round(XiaomiMapParser.convertFloat(body[2]));
                    let y2 = Math.round(XiaomiMapParser.MAX_MAP_HEIGHT - XiaomiMapParser.convertFloat(body[3]));
                    let x3 = Math.round(XiaomiMapParser.convertFloat(body[4]));
                    let y3 = Math.round(XiaomiMapParser.MAX_MAP_HEIGHT - XiaomiMapParser.convertFloat(body[5]));
                    let x4 = Math.round(XiaomiMapParser.convertFloat(body[6]));
                    let y4 = Math.round(XiaomiMapParser.MAX_MAP_HEIGHT - XiaomiMapParser.convertFloat(body[7]));

                    this.noGoArea.push([x1, y1, x2, y2, x3, y3, x4, y4]);
                }
        ...
        if (featureFlags & 0b000000000100000) {
            ...
            for (let i = 0; i < area_num; i++) {
                ...
                this.cleanArea.push([x1, y1, x2, y2, x3, y3, x4, y4]);
        ...
        if (featureFlags & 0b000100000000000) {
            ...
            try {
                ...
                console.warn("Unable to parse Pose", e);
            }
        }
        ...
        return this.convertToMap({
            ...
            no_go_area: this.noGoArea,
            clean_area: this.cleanArea
        });
    }
    ...
    convertToMap(mapContents: any) {
        ...
        if (mapContents.virtual_wall) {
            mapContents.virtual_wall.forEach((wall: any) => {
        ...
        if (mapContents.noGoArea) {
            mapContents.noGoArea.forEach((area: any) => {
        ...
        if (mapContents.cleanArea) {
            mapContents.cleanArea.forEach((area: any) => {
                entities.push(new Map.PolygonMapEntity({
                    points: area,
     ...
     * @param {any=} extra
     */
    log(name: string, extra?: any) {
        console.trace("fpos:" + (this.offset).toString(16), name, extra);
    }
    ...
    parsePose() {
        ...
        for (let i = 0; i < length; i++) {
            ...
            for (let j = 0; j < elements; j++) {
                // @ts-ignore
                pose.push(
                    XiaomiMapParser.convertInt16(this.buf.readInt16LE(this.offset)),
                    XiaomiMapParser.MAX_MAP_HEIGHT - XiaomiMapParser.convertInt16(this.buf.readInt16LE(this.offset + 2))
     ...
     * to format (mm shifted by 20m).
     *
     * @param {number} value
     */
    static convertFloat(value: number) {
        return Math.ceil((20 + value) * 100);
    }

    /**
     * Converts a location (mm shifted by 20m, upside-down) to Viomi position.
     *
     * @param {number} x
     * @param {number} y
     */
    static positionToViomi(x: number, y: number) {
        y = XiaomiMapParser.MAX_MAP_HEIGHT - y;
        return {x: x / 100 - 20, y: y / 100 - 20};
    }

    /**
     * Converts a viomi location 1/20m relative to 20m to format.
     *
     * @param {number} x
     */
    static convertInt16(x: number) { //TODO?
        return x * 5;
    }

    static convertInt16Position(x: number, y: number) {
        return {
            x: XiaomiMapParser.convertFloat(x),
            y: XiaomiMapParser.MAX_MAP_HEIGHT - XiaomiMapParser.convertFloat(y)
        };
    }
}
...
export default XiaomiMapParser;