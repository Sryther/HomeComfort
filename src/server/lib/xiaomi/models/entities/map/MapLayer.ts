...
class MapLayer extends SerializableEntity {
    private type: any;
    readonly pixels: any;
    private dimensions: { x: { min: undefined; max: undefined; mid: undefined; }; y: { min: undefined; max: undefined; mid: undefined; }; } | undefined;
    static TYPE: Readonly<{ FLOOR: string; WALL: string; SEGMENT: string }> = Object.freeze({
        FLOOR: "floor",
        WALL: "wall",
        SEGMENT: "segment"
    });
    ...
    constructor(options: any) {
        ...
        if (!options.pixels.every((e: any) => Number.isInteger(e))) {
    ...
    calculateDimensions() {
        if (this.pixels.length > 0) {
            ...
            for (let i = 0; i < this.pixels.length; i = i + 2) {
                // @ts-ignore
                if (!this.dimensions.x.min || this.pixels[i] < this.dimensions.x.min) {
                    this.dimensions.x.min = this.pixels[i];
                }

                // @ts-ignore
                if (!this.dimensions.x.max || this.pixels[i] > this.dimensions.x.max) {
                    this.dimensions.x.max = this.pixels[i];
                }

                // @ts-ignore
                if (!this.dimensions.y.min || this.pixels[i+1] < this.dimensions.y.min) {
                    this.dimensions.y.min = this.pixels[i+1];
                }

                // @ts-ignore
                if (!this.dimensions.y.max || this.pixels[i+1] > this.dimensions.y.max) {
                    this.dimensions.y.max = this.pixels[i+1];
                }
            }

            // @ts-ignore
            this.dimensions.x.mid = Math.round((this.dimensions.x.max + this.dimensions.x.min) / 2);

            // @ts-ignore
            this.dimensions.y.mid = Math.round((this.dimensions.y.max + this.dimensions.y.min) / 2);
        } else {
            // @ts-ignore
            this.dimensions = {x: { min: 0, max: 0, mid: 0 }, y: { min: 0, max: 0, mid: 0 } };
        }
    }
}
